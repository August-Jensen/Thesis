...
class ExpectationMaximization(object):
    ...
    def emission_probability(self, state, t):
        ...
        """
        if t == 0:
            previous_x = 1  # Handle the case for the first observation
        else:
            previous_x = self.data[t-1]

        mean = self.mu[state] + self.phi[state] * previous_x
        variance = self.sigma[state] ** 2
        emissions = norm.pdf(self.data[t], mean, np.sqrt(variance))
        # print(emissions)
        return  emissions
    ...
    def calculate_log_likelihood_contributions(self):
        ...
        for t in range(len(self.data)):
            ...
            for state in range(self.n_states):
                ...
                # Convert log likelihood to likelihood for summation
            ...
            log_likelihood_contributions[t] = np.log(likelihood_sum) if likelihood_sum > 0 else -np.inf
            print(log_likelihood_contributions)
    ...
    def fit(self, ): # max_iterations=100, tolerance=1e-6):
        ...
        for iteration in range(self.max_iterations):
            ...
            current_log_likelihood = np.sum(self.calculate_log_likelihood_contributions())
