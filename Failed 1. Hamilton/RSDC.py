...
def initial_RSDC_Guess(number_of_correlation_parameters):
    ...
    random_guesses = np.random.uniform(-1, 1, 2 * number_of_correlation_parameters).tolist()
...
def RSDC_log_likelihood_contribution(N, t, state, state_correlation_matrix, standard_deviations, standardized_residuals):
    """
...
def RSDC_total_log_likelihood(data, RSDC_guess, standard_deviations, standardized_residuals):
    ...
    for t in range(T):
        ...
        for state in range(n_states):
            eta[state] = RSDC_log_likelihood_contribution(N, t, state, state_correlation_matrix, standard_deviations, standardized_residuals)
...
def fit(dataframe):
    ...
    if result.success:
        ...
        RSDC_se = np.sqrt(np.diagonal(result_hessian))
        RSDC_fun = result.fun
        RSDC_jac = result.jac
        RSDC_nit = result.nit
        ...
        estimated_correlation_matrix = form_correlation_matrix(RSDC_params[2:])
